...
func _ready() -> void:
	...
	var new_x := position.x + rng.randf_range(-horizontal_move, horizontal_move)
	t.tween_property(self, "position:x", new_x, dur)
	var move_amount := rng.randf_range(0, vertical_move)
	...
	t.tween_property(self, "position:y",
			y_move + rng.randf_range(move_amount / 2, vertical_move) / 2, dur / 2)\
